...
const StartQuiz = (props) => {
    ...
    const checkValidity = (element, value) => {
        if (element === 'questions' && value <= 20)
            setInputConfig({ ...getInputConfig, questions: { ...getInputConfig[element], isValid: true } })
        else if (element === 'range' && value <= 100)
            setInputConfig({ ...getInputConfig, range: { ...getInputConfig[element], isValid: true } })
    }

    const submitAnswerHandler = (event, question) => {
        event.preventDefault();
        let correctAns = getCorrectAns(Number(question.operand1), Number(question.operand2), question.operator);
    ...
    return (
        <div className={style.Form}>
            ...
            <Quiz operand1={operand1} operand2={operand2} operator={operatorArr[operatorIndex]} submitAnswer={submitAnswerHandler} qsnNumber={qsnNumber}/> 
            :
                isQuizFinished ?
                <Result questions={questions} finalScore={finalScore} totalQsns={getInputConfig.questions.value}/>
                :
                <form onSubmit={startQuizHandler}>
                    {Object.keys(getInputConfig).map(element => (
                        <div key={element} className={style.InputDiv}>
                            ...
                            <input {...getInputConfig[element].config} value={getInputConfig[element].value} onChange={(event) => textChangeHandler(event, element)} />
                        </div>
                    )
                    )}
                    <br /><br /><br /><br />
